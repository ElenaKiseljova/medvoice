...
<section class="banner" id="banner">
  ...
  <?php if ( !is_user_logged_in(  ) ) : ?>
    <form class="banner__form" method="get" action="<?= medvoice_get_special_page( 'forms', 'url' ); ?>">
      ...
      <button class="button button--trial" type="submit"><?= __( 'Оформить триал', 'medvoice' ); ?></button>
    </form>
  <?php elseif( medvoice_user_have_subscribe() ) : ?>
  <?php else : ?>
    <a class="banner__link button" href="<?= medvoice_get_special_page( 'tariffs', 'url' ); ?>"><?= __( 'Оформить подписку', 'medvoice' ); ?></a>
  <?php endif; ?>    
</section>
